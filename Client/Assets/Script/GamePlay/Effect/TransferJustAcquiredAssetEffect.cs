...
namespace GamePlay
{
    class TransferJustAcquiredAssetEffect : EffectBase
    {
        ...
        protected override void OnTakeEffect(GamePlayer target_player)
        {
            assetCnt = player.Role.GetAssetCnt(assetMjType, assetSubType);
            if (assetCnt > 0)
            {
                // player.SendGameBuffTriggerEvent( Buff_NOTIFY_TYPE.GET_DAMAGE, this);
                player.Role.GainAsset(assetMjType, assetSubType, 0 - assetCnt);
            }
        }
    }
}
